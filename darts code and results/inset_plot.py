...
def dataframe_to_latex(df):
    ...
    #'ar': 'AR','theta': 'Theta', 'transformer': 'Transformer','sarimax': 'SARIMAX','lgbm': 'LGBM','prophet': 'Prophet','holtwinter': 'HOLTWINTER','ar': 'SARIMA'
    df.replace({'prophet': 'Prophet'}, inplace=True)
    
    # Specify columns where you want to replace 'Inf'
    #numeric_cols = df.select_dtypes(include=[np.number]).columns
    #df[numeric_cols] = df[numeric_cols].replace({sys.float_info.max: np.inf, "inf": np.inf})
    df.replace({sys.float_info.max : np.inf, 'inf': np.inf}, inplace=True)
    df.infer_objects(copy=False)
    df_melted = df.melt(id_vars=['Model type', 'Method'])
    df_melted.columns = ['Model type', 'Method', 'Metric', 'Value']
    df_melted.infer_objects(copy=False)
...
if __name__ == '__main__':
    ...
    for model_name in model_names:
        ...
        try:
            ...
            print(f"Key or learning rate not found in data dictionary: key={args.key1}, lr={args.lr1}")
        # Handle the missing key or learning rate gracefully, such as providing a default value or skipping further processing
            sets1 = None  # Assigning a default value or None as per your requirement

        # Similarly, handle the sets2 computation
        try:
            sets2 = [np.stack(data[args.key2][args.lr2]['sets'])[T_burnin+1:,0], np.stack(data[args.key2][args.lr2]['sets'])[T_burnin+1:,1]]
        except KeyError:
            print(f"Key or learning rate not found in data dictionary: key={args.key2}, lr={args.lr2}")
            # Handle the missing key or learning rate gracefully, such as providing a default value or skipping further processing
            sets2 = None  # Assigning a default value or None as per your requirement
        ...
        alpha = data['alpha']
        scores = data['scores'].iloc[T_burnin+1]
